...
| The 4P marketing mix for a launch or commercial plan, and the 4A customer-access lens (availability, affordability, acceptability, awareness) for reach questions in low-penetration or rural markets | `references/frameworks.md` |
...
| India sector primers (28: auto, aviation, cement, defence manufacturing, e-commerce, ed-tech, electric vehicles (four-wheelers), financial services, banking, FMCG, food & beverage processing, food delivery, healthcare, hospitality, insurance, IT/ITES, logistics, media & entertainment, metals & mining, oil & gas, payments, pharmaceuticals, power, quick commerce, retail, semiconductors & electronics, steel, telecom) — a 28-row overview map with a five-forces column, then per sector: value chain, KPIs with formulas, regulators and the case tension — plus, on 20 of the 28, dated size & growth, five forces, cost vs revenue drivers and key terms; and a "sizing traps across sectors" section | `references/india-sector-primers.md` |
...
| Economic value to the customer (EVC) as the price ceiling; scarcity and auction pricing for a fixed, non-replicable supply; access/tiered pricing when the objective is volume and reach rather than margin | `references/pricing-strategy.md` |
...
| Situation | Read |
|-----------|------|
| Choosing *which* case to run next, or building a study plan — every case and drill indexed by sector, case type and difficulty (Foundation/Core/Advanced), a four-week curriculum ladder, a sector × case-type coverage matrix and the thin spots | `references/case-practice-index.md` |
| Need a one-page card for a case type mid-case — keywords to restate, objective, clarifier bank, equation spine, approach buckets, the numbers that crack it, the 4A customer-access check, risks, and a close with recommendation guardrails (Cards 1–6: profitability, market entry, new product launch, pricing, M&A, growth) | `references/case-type-cheat-sheets.md` |
| Running or coaching a practice case — 19 worked, quantified cases (profitability, pricing, market entry, sizing, PE and carve-out, capacity, break-even, access pricing, returns) each with issue tree → quantified spine → recommendation → coaching notes | `references/practice-cases-quantified.md` |
| Want breadth — 47 compact case drills (setup → structure → quantified spine → aha → trap) in five parts: A1–A17 profitability & turnaround, B1–B13 market entry & new product, C1–C6 pricing & commercial structure, D1–D6 deals, defence & contract economics, E1–E5 org, operations & abstract/creative | `references/case-bank-worked.md` |
| Interview-flavoured case bank — a curated practice menu (case universe by type) plus 17 fully worked interview classics (market entry, profitability, M&A and synergies, growth, pricing, ops/org, non-traditional), each named, difficulty-rated and worked end to end | `references/case-bank-interview-classics.md` |
| Unconventional prompts no standard framework fits — U1–U8: hiring funnel, remote-plant attrition, no-capex OEE debottleneck, IT/OT integration of an acquired plant, logistics and field-sales digitisation on a fixed budget, an AI chatbot's *net* savings, an offshore-centre location matrix, subscription break-even with a growing base — plus the social-sector method (logic model, cohort-flow math, cost per additional outcome) | `references/case-bank-unconventional.md` |
| Drilling the five case reflexes — fast case math (rounding, distributive, Rule of 72, margin bridges), exhibit/chart reading (6-step protocol + chart-type cheat sheet), structuring drills with 90-second framework scaffolds, **Part D error hunt** (D1–D16: one planted flaw each — mislabelled CAGR window, stock added to a flow, wrong base, double-counted footfall, an answer that does not answer the question) and **Part E decomposition & reconciliation** (E1–E6: price-volume-mix, weighted contribution, share points to revenue, target vs capacity, rule-of-72 check, geometric-series break-even) | `references/case-cracking-drills.md` |
...
| India-market guesstimates — nine worked estimates (stock build, growth+replacement flow, capacity/route, consumption, expected value, event/occasion, stock ÷ validity period, two-method ARPU reconcile), pictorial segment trees, a benchmark-number spine with an explicit reconciliation rule (pick one set and state it), sector snapshots, and worked case drills A–S (incl. discount-curve profitability, household-device and E2W entry, ride-hail launch, ticket pricing, hospital queueing, quick-commerce share loss, tier-3 services, cohort-flow dropout, West Africa entry) | `references/india-guesstimates-and-cases.md` |
| Guesstimate drill bank organised by method — 35 worked estimates across Part A top-down, Part B ratio/benchmark, Part C supply/bottleneck, Part D two-method reconciliation and Part E physical & geometric (packing, area/density, length), each with a sanity anchor and an "error fixed" callout, plus a pre-answer error checklist | `references/guesstimate-drill-bank.md` |
| "Metric dropped X%" / root-cause prompts — the 5-gate metric-drop protocol (real? → define → segment → date → internal/external), funnel walk, 8 worked RCA archetypes (promotion change, policy change, UI + incentive change, environmental handover, partner outage, structural multi-cause decline, content/rights loss, segment adoption decay) and a signature→cause cheat sheet | `references/product-rca-casebank.md` |
...
| Choosing the alliance structure — contract vs JV vs equity stake vs acquisition — against control needed, capital at risk, reversibility and speed | `references/ecosystem-alliance-strategy.md` |
...
| Interview-classic case bank — practice menu by type + 17 worked exemplars | `references/case-bank-interview-classics.md` |
| Picking the next case or planning a study block — index of every case and drill by sector, type and difficulty, with a four-week ladder | `references/case-practice-index.md` |
| One-page cards per case type (Cards 1–6) with clarifier bank, equation spine, 4A access check and recommendation guardrails | `references/case-type-cheat-sheets.md` |
| Unconventional cases — people/talent, operations without capex, technology and AI business cases, location matrices, subscription break-even, social-sector outcomes | `references/case-bank-unconventional.md` |
| Case reflex drills — case math, exhibit reading, structuring + 90-second scaffolds, error hunt (D1–D16) and decomposition/reconciliation (E1–E6) | `references/case-cracking-drills.md` |
...
| Due Diligence (Deep Dive) | Commercial/Operational/Financial/Technical/ESG DD frameworks, EBITDA quality of earnings, red flags by category, PE vs. strategic acquirer focus, work plan templates; **sell-side DD** (§8.5) — running the diligence on your own asset first to find and fix what a buyer would discount | `references/due-diligence-deep-dive.md` |
...
| Methodology | When to Read | Reference |
|-------------|-------------|-----------|
| M&A Fundamentals & Quantitative Toolkit | Deal feasibility gates (strategic fit, affordability, regulatory clearance, integration capacity) screened before any valuation; deal-type taxonomy (merger/acquisition/asset purchase/JV/reverse merger/spin-off); 6-stage deal process; integration communication sequencing; 5 strategic-integration archetypes (vertical/horizontal/conglomerate/concentric/reverse); synergy-by-deal-type framework; DCF, LBO, and accretion-dilution primers with the accretive/dilutive P/E rule of thumb | `references/mergers-acquisitions-fundamentals.md` |
| Synergy Modeling & Validation | Bottom-up initiative-level synergy construction; confidence tiering (High/Medium/Low); phasing waterfalls; one-time cost estimation; cross-sell reality checks; synergy bridge; synergy **flow-through** — tracing a gross synergy through leakage, phasing, one-time cost and tax to the cash that actually reaches the buyer; post-close tracking cadence | `references/synergy-modeling-validation.md` |
